...
class Game {
  constructor({
    ...
    try {
      ...
      showError("Game loop is not defined.", "Create loop function. [function loop() {}] => Add loop function to game initialization. [loop: loop,]", error);
    }
  }
  internalSetup(s) {
    try {
      ...
      showError("Internal canvas setup error.", "Ensure game size are of type number and in object format. [s: { w: ~, h: ~ }]", error);
    }
  }
  begin() {
    try {
      const loop = now => {
        ...
        if (elapsed > this.requiredElapsed) {
          try {
            ...
            showError("External error in game loop.", "Check code inside of defined loop function.", error);
          }
          this.lastTime = now;
        }
      ...
      showError("Interal loop setup error. ", "Create loop function. [function loop() {}] => Add loop function to game initialization. [loop: loop,]", error);
    }
  }
  end() {
    this.running = false;
...
class Object {
  ...
  loadImg() {
    try {
      ...
      errorBox("Unable to load image for sprite " + this.name + ".", "Check that source path is correct.", error);
    }
  }
  add({
    name,
  ...
  move({
    ...
    try {
      ...
      errorBox("Unable to run move function on sprite " + this.name + ".", "Ensure that parameter is of type number.", error);
    }
  }
  draw() {
    try {
      if (this.img === undefined) {
        ...
        if (this.imgLoaded) {
          if (this.s.r) {
            ctx.drawImage(this.img, this.pos.x - this.s.r, this.pos.y - this.s.r, this.s.r * 2, this.s.r * 2);
          } else {
...
class ParticleSystem {
  ...
  loadImg() {
    try {
      ...
      errorBox("Unable to load image for particle.", "Check that source path is correct.", error);
    }
  }
  init() {
    try {
      ...
      showError("Internal particle initialization error.", "Ensure parameters are of propper type.", error);
    }
  }
  move() {
    try {
      ...
      showError("Internal particle move error.", "Ensure friction is of type number.", error);
    }
  }
  draw() {
    try {
      if (this.img === undefined) {
        ...
        if (this.imgLoaded) {
          this.pieces.forEach(piece => {
            ctx.drawImage(this.img, piece.pos.x, piece.pos.y, piece.s.w, piece.s.h);
          });
        }
      }
    } catch (error) {
      showError("Internal particle draw error.", "Ensure parameters are of propper type.", error);
    }
  }
  update() {
    this.move();
  ...
  try {
    if (dir === "to") {
      if (type === "axis") {
        ...
        let angle = Math.atan2(object2.pos.y - object1.pos.y, object2.pos.x - object1.pos.x);
    ...
    showError(`Error routing ${object1.name} ${dir} ${object2.name}.`, "Ensure speed parameter is passes. [{ speed: ~ }] Ensure objects have pos and s. [pos: { x: ~, y: ~ }, s: { -type=rect-(w: ~, h: ~) -type=circle-(r: ~) }]", error);
  }
}
function collide({
  object1,
  ...
  try {
    if (type === "rect") {
      return object1.pos.x + object1.s.w >= object2.pos.x && object1.pos.x <= object2.pos.x + object2.s.w && object1.pos.y + object1.s.h >= object2.pos.y && object1.pos.y <= object2.pos.y + object2.s.h;
    } else if (type === "circle") {
      return Math.abs(object1.pos.x - object2.pos.x) <= object1.s.r + object2.s.r && Math.abs(object1.pos.y - object2.pos.y) <= object1.s.r + object2.s.r;
    }
  } catch (error) {
    showError(`Error colliding ${object1.name} and ${object2.name}.`, "Ensure objects have pos and s. [pos: { x: ~, y: ~ }, s: { -type=rect-(w: ~, h: ~) -type=circle-(r: ~) }]", error);
  }
}
module.exports = {
  Game: Game,
